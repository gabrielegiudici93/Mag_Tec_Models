...
\begin{table}[H]
    \centering
    \caption{Datasets collected on 11~Nov~2025. Metrics will be inserted from the JSON reports after validation.}
    \begin{tabularx}{\linewidth}{@{}lcc>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{2.8cm}>{\raggedright\arraybackslash}X@{}}
        ...
        Multiple\_Points/2.5mm\_single\_test1 & Multi-point & 0, 10, 20\% & centre + offsets 4–12 & models/, metrics JSON & Extended neighbourhood offsets (collection in progress). \\
        \bottomrule
    \end{tabularx}
    \label{tab:datasets-collected}
\end{table}

The subsequent sections present the quantitative results for these two runs. Values denoted as
\texttt{TBD} will be filled in automatically when the evaluation script is rerun after data
...
\subsection{Force Mapping Metrics}
\paragraph{Single-point (2.5mm\_single\_test1).}
\begin{table}[H]
    \centering
...
\paragraph{Multi-point (Multiple\_Points/2.5mm\_single\_test1).}
\begin{table}[H]
    \centering
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lcccccc@{}}
        ...
        Stretch & Samples & RMSE [N] & STD [N] & $\Delta F_{\text{min}}$ [N] & KPM1 & KPM2 \\
        \midrule
        stretch\_000pct & TBD & TBD & TBD & TBD & TBD & TBD \\
        stretch\_010pct & TBD & TBD & TBD & TBD & TBD & TBD \\
        stretch\_020pct & TBD & TBD & TBD & TBD & TBD & TBD \\
        combined (pooled) & TBD & TBD & TBD & TBD & TBD & TBD \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lccc@{}}
        ...
        \midrule
        stretch\_000pct & TBD & TBD & Centre + offsets 4–12 \\
        stretch\_010pct & TBD & TBD & --- \\
        stretch\_020pct & TBD & TBD & --- \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{Pooled offset pipelines across all stretches (combined datasets).}
    \label{tab:offset-combined}
    \begin{tabular}{@{}lccc@{}}
        \toprule
...
\subsection{Stretch Classification}
\begin{table}[H]
    \centering
    \caption{Stretch classification metrics (pooled data).}
    \begin{tabular}{@{}lccc@{}}
        \toprule
        Metric & Full sensors & Central subset & Notes \\
        \midrule
        Samples & TBD & TBD & Random Forest (250 trees) \\
        Accuracy & TBD & TBD & Will be updated from JSON report \\
        \bottomrule
    \end{tabular}
\end{table}

\subsection{Discussion}
The latest evaluation (50 presses per offset, three stretch levels) indicates:
For the runs collected on 11~November~2025 the automated pipeline will populate the tables above
once the operator triggers the evaluation step. The discussion below will be expanded after the JSON
reports are ingested. In the meantime the following observations remain valid:
\begin{itemize}[leftmargin=1.5em]
    \item The single-point configuration achieved RMSE below \SI{0.05}{\newton} and satisfied KPM2 for
          0~\% and 10~\% stretch. At 20~\% the variance increased ($\mathrm{STD}\approx\SI{0.064}{\newton}$),
          indicating that the prototype becomes noisier at high elongation.
    \item Offset classification for the single-point dataset reached 100\% accuracy across all stretches,
          both with the full 15-sensor feature vector and with the five-sensor neighbourhood subset.
    \item Combined models trained on all three stretches remain perfect for offset and stretch
          classification, providing confidence that the same architecture can be reused for the multi-point
          dataset.
    \item Thanks to the run-folder organisation, each dataset carries its metrics JSON and the trained
          models, so future comparisons with the upcoming multi-point run can be performed without
          ambiguity.
\end{itemize}

\subsection{Visual Summaries}
\label{sec:visual-summaries}
Figures~\ref{fig:stretch-000-summary}--\ref{fig:stretch-020-summary} show the time-series traces
exported from the latest run (generated via \texttt{plot\_single\_point\_stretch.py}, folder
\texttt{plots/single\_point/run\_20251110\_185744/}). Each stretch produces five figures (centre,
NW, NE, SW, SE offsets); the examples below focus on the centre press set. The first three subplots
display the raw force components (Fz, Fx, Fy) with shared axis limits, while the bottom subplot
reports the baseline-subtracted Sensor~8 channels \((B_x, B_y, B_z)\), rescaled to \([-1, 1]\) for
visual clarity. Alignment of time and axis ranges across stretches supports direct comparison between
elongation conditions. Additional offset-specific figures follow the naming convention
\texttt{stretch\_XYZ\_<offset>\_presses.png}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{../plots/single_point/run_20251110_185744/stretch_000pct_center_presses.png}
\caption{Stretch~0\% (centre offset): raw force components and the normalised Sensor~8 channels
        aggregated across 49 presses.}
    \label{fig:stretch-000-summary}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{../plots/single_point/run_20251110_185744/stretch_010pct_center_presses.png}
\caption{Stretch~10\% (centre offset): force envelopes remain within KPM limits; the
        \(B_x,B_y,B_z\) traces stay phase-aligned with the force profile.}
    \label{fig:stretch-010-summary}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{../plots/single_point/run_20251110_185744/stretch_020pct_center_presses.png}
\caption{Stretch~20\% (centre offset): despite larger elongation, the Sensor~8 channels follow the
        same temporal signature, confirming robustness of the normalisation pipeline.}
    \label{fig:stretch-020-summary}
\end{figure}
...
\begin{description}[leftmargin=1.5em]
    ...
    \item[Combined stretch-gated pipeline.] A stretch classifier routes each sample to the pooled or
          ...
          Accuracy remains 100\% (Table~\ref{tab:offset-combined}).
...
\begin{description}[leftmargin=1.5em]
    \item[Per-stretch force regressors.] The multi-point wrapper will train three regressors once
          the dataset \texttt{Multiple\_Points/2.5mm\_single\_test1} is collected. Metrics will be
          inserted into Table~\ref{tab:force-multi} automatically.
    \item[Per-stretch offset classifiers.] The same evaluation script will extend the label space to
          include offsets 4--12 and report the per-stretch accuracies in Table~\ref{tab:offset-multi}.
    \item[Combined stretch-gated pipeline.] A shared stretch detector plus pooled offset classifier
          will provide the final prediction, mirroring the single-point architecture.
    \item[Training invocation.] Once data collection finishes, run
\begin{verbatim}
